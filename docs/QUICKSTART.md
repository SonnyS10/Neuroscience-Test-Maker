...
### 8. Export for Other Platforms (Optional)
Want to use your test in EEGLAB or E-Prime?

**Method 1: Save As with Extension**
1. Go to **File > Save Test As...**
2. Name your file with the appropriate extension:
   - `mytest_eeglab.txt` for EEGLAB format
   - `mytest_eprime.txt` for E-Prime format
   - `mytest.json` for native format (default)

**Method 2: Export Menu**
1. Go to **File > Export As**
2. Choose your target format:
   - **EEGLAB Format (.txt)** - For EEG data analysis
   - **E-Prime Format (.txt)** - For experimental delivery
   - **JSON Format (.json)** - Native format

See [EXPORT_FORMATS.md](EXPORT_FORMATS.md) for detailed information about each format.
